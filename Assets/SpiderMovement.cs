...
public class SpiderMovement : MonoBehaviour {
	...
	void FixedUpdate() {
		if(Input.GetKey(KeyCode.RightArrow) && !objectToRight()) {
			...
			Vector3 newVel = new Vector3(oldVel.x, oldVel.y, Mathf.Lerp(oldVel.z, maxVel, easing));
	...
	bool isOnGround() {
		...
		Vector3 toLeftCorner = new Vector3(0f, -GetComponent<Collider>().bounds.extents.y, -GetComponent<Collider>().bounds.extents.z + 0.2f);
		Vector3 toRightCorner = new Vector3(0f, -GetComponent<Collider>().bounds.extents.y, GetComponent<Collider>().bounds.extents.z - 0.2f);
	...
	bool objectToLeft() {
		...
		bool left = Physics.Raycast(transform.position, Vector3.back, GetComponent<Collider>().bounds.extents.z + 0.1f);
		bool diagUp = Physics.Raycast(transform.position, toTopCorner, toTopCorner.magnitude + 0.1f);
		bool diagDown = Physics.Raycast(transform.position, toBottomCorner, toBottomCorner.magnitude + 0.1f);
